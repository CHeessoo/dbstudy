...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 1, 1, TO_DATE('20/07/01', 'YY/MM/DD')); --기본 날짜포맷
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 3, 2, TO_DATE('20/07/03'));  -- 날짜포맷 생략
...
INSERT INTO ORDER_T(ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 6, 4, TO_DATE('20/07/10'));
 ...
 --오라클의 날짜 구분 기본구문이 /(슬래시)로 되어 있어서 날짜해석형식(포맷) 생략가능
 --원래형식 TO_DATE('20/01/01','YY/MM/DD')   -> 포맷 생략 TO_DATE('20/01/01')
...
SELECT *
  FROM BOOK_T
 WHERE BOOK_NAME LIKE CONCAT(CONCAT('%', '올림픽'), '%');
...
SELECT O.ORDER_ID  AS 주문번호
 ...
 -- INSERT에서 이미 ORDER_DT에 TO_DATE처리를 했기 때문에 여기 WHERE에서 TO_DATE(O.ORDER_DT)명시가 재차 필요치 않다.
...
-- 조인으로 조회
SELECT C.CUST_NAME AS 고객명, O.ORDER_ID
...
SELECT O.ORDER_ID  AS 구매번호
 ...
 WHERE O.ORDER_DT NOT BETWEEN TO_DATE('20/07/05') AND TO_DATE('20/07/07');
...
SELECT C.CUST_NAME AS 고객명
     , COUNT(O.ORDER_ID) AS 구매도서수  -- 주문을 몇 번 했는가? (구매한 횟수)
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;   -- 고객이름이 중복의 가능성이 있기 때문에 고객번호와 이름이 모두 같아야 같은사람으로 볼 수 있도록 그룹핑.
 
 
-- '김연아'가 구매한 책이 몇 종인가? (피겨 교본을 2번 주문했다면 구매한 책은 1종)
SELECT C.CUST_NAME AS 고객명
     , COUNT(DISTINCT O.BOOK_ID) AS 구매한책종류   -- 같은 책 번호 중복제거
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
...
SELECT C.CUST_NAME AS 고객명
  ...
  FROM BOOK_T B RIGHT OUTER JOIN ORDER_T O
    ON B.BOOK_ID = O.BOOK_ID RIGHT OUTER JOIN CUSTOMER_T C
    ON C.CUST_ID = O.CUST_ID
 GROUP BY C.CUST_ID, C.CUST_NAME
