/*
    ...
    2. 1    :   M
       PK   :   FK
       부모 :   자식
    3. 반드시 부모 테이블을 먼저 만들고, 자식 테이블은 나증에 만들어야 한다.
    4. 반드시 자식 테이블을 먼저 지우고, 부모 테이블을 나중에 지워야 한다.
*/

/*
    삭제 옵션
    1. ON DELET CASCADE  : 외래키가 참조하는 기본키 값이 삭제되면 외래키도 함께 삭제한다.
    2. ON DELET SET NULL : 외래키가 참조하는 기본키 값이 삭제되면 외래키를 NULL로 처리한다. 
*/
