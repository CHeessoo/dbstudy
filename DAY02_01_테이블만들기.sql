...
/*
    ...
    1. NUMBER(p,s) : 정밀도 p, 스케일 s로 표현하는 숫자 형식
        ...
        2) 스케일 s : 소수부의 유효 숫자의 갯수 (예: 0.1->1 , 0.01->2 , 0.10->1)
    ...
    3. VARCHAR2(size) : 가변 문자
        ...
        2) size :  최대 4000 byte
    4. CLOB : 큰 텍스트 타입 (최대 4GB/ 4000바이트를 넘어가는 텍스트를 저장)
    5. DATE : 날짜와 시간을 동시에 표현하는 타입 (년, 월, 일, 시 , 분, 초)
    6. TIMESTAMP : 날짜와 시간을 동시에 표현하는 타입 (년, 월, 일, 시, 분, 초, 마이크로초(백만분의 1초))  -> DATE보다 정밀한 타입
*/
