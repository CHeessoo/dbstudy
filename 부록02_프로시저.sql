/*
    ...
    2. "EXECUTE 프로시저명()" 형식으로 실행할 수 있다. (프로시저 호출)  -- EXEC 최대 4글자까지 줄여서 사용 가능
    3. 형식
...
CREATE OR REPLACE PROCEDURE MY_PROC_O3(EMPLOYEEID IN EMPLOYEES.EMPLOYEE_ID%TYPE)   -- 프로시저로 전달된 인수를 저장하는 입력(IN) 파라미터(입력 파라미터 IN : 오라클의 프로시저에서만 있는 개념, 생략가능)
IS
    FIRSTNAME EMPLOYEES.FIRST_NAME%TYPE;
    LASTNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN 
...
EXECUTE MY_PROC_O3(204);    -- 프로시저로 전달하는 인수 206


-- 프로시저로부터 값을 받아오는 방법
-- 사원번호가 100번인 사원의 이름을 받아오는 프로시저
CREATE OR REPLACE PROCEDURE MY_PROC_04(FNAME OUT EMPLOYEES.FIRST_NAME%TYPE, LNAME OUT EMPLOYEES.LAST_NAME%TYPE) -- FNAME, LNAME은 프로시저 외부로 값을 반환하는 출력(OUT) 파라미터
AS
BEGIN
    SELECT FIRST_NAME, LAST_NAME
      INTO FNAME, LNAME
...
BEGIN 
    -- (프로시저로 변수를 넘겨서 요청)
    MY_PROC_04(FIRSTNAME, LASTNAME);        -- FIRSTNAME, LASTNAME 변수에 결과를 저장해달라. PLSQL에서 직접 프로시저를 호출할 땐 EXECUTE를 생략한다.
    DBMS_OUTPUT.PUT_LINE(FIRSTNAME || ' ' || LASTNAME);
END;
...
BEGIN
BUY_PROC(1, 1000, 10);
END;        -- EXECUTE 를 생략하는 방법 : PLSQL에 넣기
