...
CREATE TABLE CUST_T (
  ...
  , CONSTRAINT FK_BANK_CUST FOREIGN KEY(BANK_CODE) REFERENCES BANK_T(BANK_CODE) ON DELETE CASCADE
);
...
-- 2)
INSERT INTO CUST_T(CUST_NO, CUST_NAME, CUST_TEL, CUST_BORN, BANK_CODE) VALUES(CUST_SQ.NEXTVAL, '고길동', '01011111111', TO_DATE('95/10/05'), 'BANK003');  -- 날짜 포맷 생략
...
SELECT CUST_NO, CUST_NAME, CUST_TEL, CUST_BORN, BANK_CODE
  FROM CUST_T
 WHERE CUST_BORN = (SELECT MIN(CUST_BORN)
                      FROM CUST_T);
